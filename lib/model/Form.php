  ...
  public static function render($form_id, $vars = array())
  {
    ...
    ?>
      ...
      <div class="form-row" id="question_<?php echo $row->id; ?>" <?php if($row->visibleIf()) { 
        ...
        if ($form->id == 1 || $_SESSION['booking']['answers'][$question_id] != $answer_id) {  
      ...
      switch ($row->fieldtype) {
        case 'radio':
          foreach ($row->answers() as $key => $answer) { 
            ...
            ?>
            <input <?php echo $checked; ?> onclick="mutateTotal(<?php echo $row->id; ?>, '<?php echo $mutation->mutation; ?>', '<?php echo $mutation->type; ?>'<?php if ($form_id == 1) { ?>,'' , true<?php } ?>);" type="radio" name="answer_<?php echo $row->id; ?>" value="<?php echo $key; ?>" id="answer_<?php echo $row->id; ?>_<?php echo $key; ?>" style="width:auto;"> <label for="answer_<?php echo $row->id; ?>_<?php echo $key; ?>"><?php echo $answer; ?></label><br />
          <?php 
          }
          break;
        ...
        case 'select':
          ?>
          <select style="width:450px;" onchange="mutateTotal(<?php echo $row->id; ?>, '<?php echo $mutation->mutation; ?>', '<?php echo $mutation->type; ?>'<?php if ($form_id == 1) { ?>,'', true<?php } ?>);" name="answer_<?php echo $row->id; ?>" id="answer_<?php echo $row->id; ?>_<?php echo $key; ?>" style="width:auto;">
          <?php
          foreach ($row->answers() as $key => $answer) { 
            $mutation = $mutations[$key];
            $checked = '';
        ...
        case 'input':
            ...
            <input onkeyup="mutateTotal(this.value, '<?php echo $mutation->mutation; ?>', '<?php echo $mutation->type; ?>', <?php echo $row->id; ?><?php if ($form_id == 1) { ?>, true<?php } ?>);" type="text" name="answer_<?php echo $row->id; ?>" id="answer_<?php echo $row->id; ?>_<?php echo $key; ?>" style="width:450px;"><br />
          <?php 
          break;
      }
       ?>
...
<div id="pricing-info">
  <p>De totaalprijs wordt <strong>€ <span id="total-sum"><?php echo number_format($total,2,',','.'); ?></span></strong></p>
</div>      
<?php } ?>
<?php
    }
