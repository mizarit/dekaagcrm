...
Widget.prototype =
{
  ...
  { 
    //console.log(where);
	  ...
	  if (what == 'facility') {
		  category = $('widget-facility').options[$('widget-facility').options.selectedIndex].innerHTML.replace('&amp;', '**');
        ...
        tbody.insert(tr);
            ...
            if(jQuery(i).css('display') == 'block') {
              ...
              params['answer_'+q] = v;
			    ...
			    if (widget.options.duration[appTypeId]['Buffer'] > 0) {
			      td4.innerHTML = widget.options.duration[appTypeId]['Buffer']+' dagen';
			    }
			    else {
            var h = Math.floor(widget.options.duration[appTypeId]['Duration'] / 60);		      
            if (h > 0) {
              m = parseInt(widget.options.duration[appTypeId]['Duration'])  - (h*60);
              if (m > 0) {
                td4.innerHTML = h + ' uur en '+m+' minuten';
              }
              else {
                td4.innerHTML = h +' uur';
              }
              ...
              td4.innerHTML = widget.options.duration[appTypeId]['Duration']+' minuten';
            }
			    }
